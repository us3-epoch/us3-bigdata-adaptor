...
public class UFileFileSystem extends org.apache.hadoop.fs.FileSystem {
    ...
    public void initialize(URI name, Configuration conf) throws IOException {
        ...
        try {
            ...
            if (cfg.isUseMDS()) {
                objcfg = new ObjectConfig("http://"+ cfg.getMDSHost());
            ...
            /** 给SDK库安装拦截器 */
            UFileInterceptor.Install(cfg.getLogLevel());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private boolean ufileRename(String srcKey, String dstKey) throws IOException {
        ...
        try {
            BaseResponseBean response = UfileClient.object(objauth, objcfg)
    ...
    private ListUFileResult listUFileStatus(String prefix, String nextMark) throws IOException {
        ...
        try {
             ObjectListWithDirFormatBean response = UfileClient.object(objauth, objcfg)
                     ...
                     .withPrefix(prefix)
                     .withMarker(nextMark)
                     .dataLimit(Constants.LIST_OBJECTS_DEFAULT_LIMIT)
                     .execute();
    ...
    private UFileFileStatus innerSampleGetFileStatus(Path path, OSMeta osMeta) throws IOException {
        ...
        /** 1. 尝试从Cache中获取 */
        UFileFileStatus fs = Constants.ufileMetaStore.getUFileFileStatus(cfg, osMeta.getKey());
        if (fs != null) {
            if (fs.is404Cache()) {
